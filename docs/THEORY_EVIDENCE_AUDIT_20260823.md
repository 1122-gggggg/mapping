...
| P2 | LFOE, MP-SfM, Detector-Free SfM, PixSfM, RoMo, Light3R-SfM, depth-guided SfM | These papers target translation outliers, low overlap/parallax, detector failure, keypoint noise, dynamic content, feed-forward cost, or small-parallax movies. | **Experiment-only.** LFOE is blocked without an explicit license grant; DFSfM/PixSfM require sealed custom environments; every backend must pass S6/S9, gauge, resource, and provenance gates before promotion. |
...
| S4 repeated-structure/outlier control | Doppelgangers++, G-MASt3R-SfM GVP, Planar-SfM consistency | `stage_doppelgangers`, `stage_verify_pairs`, `audit_dg_graph.py` | Doppelgangers++ rejects aliases but is CC BY-NC-SA; deployment terms must permit it. Dense H/E agreement and deterministic largest-component pruning remain independent hard gates. |
| S5 reconstruction and refinement | COLMAP GlobalMapper, LFOE, GLUEMAP, PixSfM, Detector-Free SfM, MP-SfM, RoMo | `stage_glomap`, `run_s0_s9.py`, `finalize_edm_model.py` | Maintained BSD-licensed COLMAP 4.x GlobalMapper is the single default. LFOE has no license grant and is experiment-only. Fixed calibration, isolated database clone, S6/S9, resource and provenance gates remain unchanged. |
| S5.7 bridge, Sim(3), and gauge | Umeyama, RANSAC, COLMAP GlobalMapper, 1DSfM, g2o | `admission.py::evaluate_geometry_authority`, `update_quality_gates.py::bridge_quality_checks`, `audit_independent_sim3.py` | Independent per-sequence COLMAP GlobalMapper models supply bridge geometry while preserving the modern database schema. Fusion authority still requires disjoint support; site cutoffs cannot replace S9. |
