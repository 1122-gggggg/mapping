...
| P2 | MP-SfM, Detector-Free SfM, PixSfM, RoMo, Light3R-SfM, depth-guided SfM | Each paper targets a diagnosed regime (low overlap/parallax, detector failure, keypoint noise, dynamic content, feed-forward cost, or small-parallax movies). Liu et al. 2022 explicitly tune COLMAP++ separately and exclude heavy blur. | **Deferred, experiment-only.** Trigger by diagnosed failure only. LFOE is no longer in this queue: its command-compatible global mapper is integrated, while every output still must pass the same S6/S9, gauge, resource, and artifact-provenance gates. |
...
| S4 repeated-structure/outlier control | Doppelgangers++, G-MASt3R-SfM GVP, Planar-SfM consistency | `stage_doppelgangers`, `stage_verify_pairs`, `audit_dg_graph.py` | Doppelgangers++ rejects aliases; dense H/E rotation agreement rescues supported planar edges; deterministic largest-component pruning removes outlier view groups. Component coverage and S6/S9 remain hard gates. |
| S5 reconstruction and refinement | LFOE-GlobalSfM, COLMAP, GLOMAP, GLUEMAP, PixSfM, Detector-Free SfM, MP-SfM, RoMo | `stage_glomap`, `run_s0_s9.py`, `finalize_edm_model.py` | LFOE is the single default global mapper and standard GLOMAP is an explicit alternative. Fixed-calibration, S6/S9, resource and provenance gates remain unchanged; other learned backends remain experiment-only. |
| S5.7 bridge, Sim(3), and gauge | Umeyama, RANSAC, LFOE, 1DSfM, g2o, multi-session SLAM | `admission.py::evaluate_geometry_authority`, `update_quality_gates.py::bridge_quality_checks`, `audit_independent_sim3.py` | Independent per-sequence LFOE models supply bridge geometry. Fusion authority is still per exact-pair edge with ≥2 independent disjoint-holdout groups; site cutoffs cannot replace S9. |
