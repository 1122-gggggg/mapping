#!/usr/bin/env python3
"""S2 -- bounded Fuhe probe extraction. INTER_AREA resize, no undistortion.

Each source frame is downsampled once from 3840x2160 to 1920x1080 with
``cv2.INTER_AREA``. The fixed camera is defined at that same working resolution.
...
WHY ONE WORKING RESOLUTION
  Pixel dimensions and the fixed PINHOLE camera stay identical on disk. This
  keeps downstream keypoint scaling isotropic while bounding the probe to 240
  images and exactly one camera.
...
from ts_common import (  # noqa: E402
    BUILD, BUILD_BY_SEQ, FRAMES_PER_BUILD_VIDEO, PROBE_FRAME_COUNT, RUNS, RUN_ID,
    TEST, WORKING_HEIGHT, WORKING_WIDTH, Gate, log, make_lineage_record, read_json,
...
EXPECTED_FRAME_COUNTS = {
    video.seq: FRAMES_PER_BUILD_VIDEO for video in BUILD
}
OUTPUT_WIDTH, OUTPUT_HEIGHT = WORKING_WIDTH, WORKING_HEIGHT
...
def _evenly_spaced(
    values: list[tuple[float, str]], count: int
) -> list[tuple[float, str]]:
    if count <= 0:
        return []
    if len(values) < count:
        raise ValueError(f"cannot select {count} values from {len(values)}")
    indexes = np.linspace(0, len(values) - 1, count, dtype=int)
...
def cap_hover_and_backfill(
    ...
    target_n = EXPECTED_FRAME_COUNTS.get(seq, len(planned))
    structural = [item for item in planned if item[1] != "hover"]
    selected_t = {round(t, 6) for t, _ in structural}
    structural.extend(
        item
        for item in [
        ...
        if round(item[0], 6) not in selected_t
    )
    deduplicated = {
        round(float(timestamp), 6): (float(timestamp), motion_class)
        for timestamp, motion_class in structural
    }
    structural = sorted(deduplicated.values(), key=lambda item: item[0])
    if len(structural) >= target_n:
        selected = _evenly_spaced(structural, target_n)
    else:
        selected = list(structural)
        selected_t = {round(t, 6) for t, _ in selected}
        hover = [
            item for item in planned
            if item[1] == "hover" and round(item[0], 6) not in selected_t
        ]
        max_hover = math.floor(MAX_HOVER_RATIO * target_n)
        selected.extend(_evenly_spaced(hover, min(len(hover), max_hover)))
        if len(selected) < target_n:
            raise ValueError(
                f"{seq}: only {len(selected)} safe probe timestamps for {target_n} slots"
            )
...
def to_working_resolution(image: np.ndarray) -> np.ndarray:
    """Apply the P0 pixel transform: exact INTER_AREA resize, never undistort."""
    ...
    if width * OUTPUT_HEIGHT != height * OUTPUT_WIDTH:
        raise ValueError(
            f"source image {width}x{height} does not share the 16:9 working aspect"
        )
    return cv2.resize(
        image,
...
def extract(
    ...
    for i, (t, img) in enumerate(
        s1.decode_at(v, want, gray=False, width=OUTPUT_WIDTH)
    ):
        img = to_working_resolution(img)
        h, w = img.shape[:2]
        if (w, h) != (OUTPUT_WIDTH, OUTPUT_HEIGHT):
            raise SystemExit(
                f"{seq}: adapted {w}x{h}, expected {OUTPUT_WIDTH}x{OUTPUT_HEIGHT}"
            )
        n += 1
...
def locked_source_lineage(corpus: dict) -> dict[str, dict[str, str]]:
    """Revalidate every S0 BUILD source hash once, then reuse per frame."""
...
def prove_no_undistortion(gate: Gate, images_dir: Path, frames: list[dict],
    ...
    for f in picks:
        ...
        raw = next(img for _t, img in s1.decode_at(
            v, np.array([f["t"]]), gray=False, width=OUTPUT_WIDTH))
        raw = to_working_resolution(raw)
        written = cv2.imread(str(images_dir / f["name"]), cv2.IMREAD_COLOR)

        s = OUTPUT_WIDTH / CHARUCO_BASE_W
...
def main() -> None:
    ...
    ap.add_argument("--candidate", default="fuhe_v2_fixed", choices=["fuhe_v2_fixed"])
    args = ap.parse_args()
    ...
    for v in BUILD:
        ...
        log(f"  {v.seq}: {len(fs):4d} frames  {OUTPUT_WIDTH}x{OUTPUT_HEIGHT}  "
    ...
    g.check(
        ...
        f"exact {len(BUILD)}-source locked lineage and exact manifest/disk JPEG set; no held-out source"
    ...
    # G2.2 -- every written frame is 16:9 and at the fixed working size.
    bad = [
    ...
    g.check("G2.3a", shapes == [(OUTPUT_WIDTH, OUTPUT_HEIGHT)],
    ...
    g.check("G2.4", n == PROBE_FRAME_COUNT,
            f"bounded P0 probe selects exactly {n} frames",
            n_frames=n, expected_frames=PROBE_FRAME_COUNT,
