...
def test_release_stage_required_id_sets_are_exact() -> None:
    ...
    assert required_check_ids("S4_doppelgangers") == {
        "G4.0",
        "G4.1",
        "G4.2",
        "G4.3",
    ...
    assert required_check_ids("S5_fixed_intrinsics") == {
        "G5.1",
        "G5.2",
        "G5.3",
        "G5.4",
    ...
    assert required_check_ids("S5_7_independent_sim3") == {"G5.7"}
    assert required_check_ids("S5_7_S6_geometry") == {
        "G5.7",
        "G6.1",
        "G6.2",
        "G6.3",
...
def test_recursive_gate_freshness_rejects_ancestor_source_drift(
    tmp_path: Path,
) -> None:
    first = make_gate(tmp_path / "first", {"G1"})
    first.check("G1", True, "ancestor evidence", count=1)
    first_path = tmp_path / "run" / "gates" / "first.json"
    first.write(tmp_path / "run", output_path=first_path, fail_hard=False)

    second = make_gate(tmp_path / "second", {"G2"})
    second.record_predecessor_gate("first", first_path, expected_stage="test_stage")
    second.check("G2", True, "descendant evidence", count=1)
    second_path = tmp_path / "run" / "gates" / "second.json"
    second.write(tmp_path / "run", output_path=second_path, fail_hard=False)

    assert assert_gate_chain_fresh(second_path) is True

    (tmp_path / "first" / "input.json").write_text("drift\n", encoding="utf-8")
    with pytest.raises(GateFreshnessError, match="sha256 drift"):
        assert_gate_chain_fresh(second_path)


def test_gate_can_embed_stage_metrics_without_losing_v2_material_hash(
    tmp_path: Path,
) -> None:
    gate = make_gate(tmp_path, {"G1"})
    gate.check("G1", True, "metric evidence", count=2)

    result = gate.write(
        tmp_path / "run",
        fail_hard=False,
        stage_metrics={"accepted_pairs": 7},
    )

    assert result["schema_version"] == "sfm-gate-v2"
    assert result["stage_metrics"] == {"accepted_pairs": 7}
    assert assert_gate_fresh(tmp_path / "run" / "gates" / "test_stage.json")
