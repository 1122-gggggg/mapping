#!/usr/bin/env python3
"""Initialize and independently verify the target_site S0--S3 release chain."""
...
def _typed_predicate(stage: str, gid: str, metrics: Mapping[str, Any]) -> bool:
    ...
    if stage == "S0_corpus":
        if gid == "G0.1a":
            ...
            return metrics["n_build"] == 7 and metrics["mismatched"] == []
        if gid == "G0.1b":
            _require(metrics, "n_hashed", "n_unique")
            return metrics["n_hashed"] == 9 and metrics["n_unique"] == 9
        if gid == "G0.2a":
            _require(metrics, "build_paths", "test_paths", "path_overlap")
            return (
                len(metrics["build_paths"]) == 7
                and len(metrics["test_paths"]) == 2
                and metrics["path_overlap"] == []
            )
        if gid == "G0.2b":
            _require(metrics, "n_build_hashes", "n_test_hashes", "hash_overlap")
            return (
                metrics["n_build_hashes"] == 7
                and metrics["n_test_hashes"] == 2
        ...
        if gid == "G0.4":
            ...
            return metrics["dropped"] == ["base/map/A-B.MP4"]
        if gid == "G0.5":
            _require(
                metrics,
                "excluded",
                "excluded_under_input_roots",
                "declared_inputs_under_excluded",
                "p071_lineage",
            )
            return (
                bool(metrics["excluded"])
                and metrics["excluded_under_input_roots"] == []
                and metrics["declared_inputs_under_excluded"] == []
                and metrics["p071_lineage"] == []
            )
        if gid == "G0.6":
            _require(metrics, "regex")
            return metrics["regex"] == r"^S[0-9]+"
        if gid == "G0.7":
            _require(metrics, "groups")
            groups = metrics["groups"]
            return isinstance(groups, dict) and len(groups) == 3 and sum(
        ...
        if gid == "G0.8":
            ...
            return (
                ...
                and len(directions) == 7
                and set(directions.values()) >= {"fwd", "rev", "unknown"}
    ...
    if stage == "S1_motion":
        ...
        if gid == "G1.4a":
            ...
            return len(metrics["resolved"]) == 7 and metrics["unresolved"] == []
        if gid == "G1.4b":
        ...
        if gid == "G1.4c":
            ...
            return len(values) == 7 and values.count("fwd") == 4 and values.count("rev") == 3
        if gid == "G1.5":
            _require(metrics, "n_slews", "slews", "degenerate")
            return metrics["n_slews"] == len(metrics["slews"]) and metrics["n_slews"] >= 2 and metrics["degenerate"] == []
        if gid == "G1.6":
    ...
    if stage == "S2_extract":
        if gid == "G0.2":
            ...
            return (
                len(metrics["expected_lineage"]) == 7
        ...
        if gid == "G2.2":
            ...
            return metrics["n_frames"] == 1414 and all(metrics[name] == [] for name in ("bad_dimensions", "unreadable", "hash_mismatches"))
        if gid == "G2.3a":
            _require(metrics, "shapes")
            return set(metrics["shapes"]) == {"1920x1080", "2688x1512", "3840x2160"}
        if gid == "G2.4":
            _require(metrics, "n_frames", "est_hours")
            return 700 <= metrics["n_frames"] <= 2200 and _finite(metrics["est_hours"]) > 0
        if gid == "G2.5":
            _require(metrics, "roles", "bad_role_records")
            roles = metrics["roles"]
            return isinstance(roles, dict) and sum(roles.values()) == 1414 and set(roles) <= {"triangulation", "bridge_only"} and metrics["bad_role_records"] == []
        if gid == "G2.6":
            _require(metrics, "seeded", "missing_seed_names", "extra_seed_names", "seed_model_sha256")
            hashes = metrics["seed_model_sha256"]
            return (
                set(metrics["seeded"]) == {"1920x1080", "2688x1512", "3840x2160"}
    ...
    if stage == "S2b_intrinsics":
        if gid == "G2.7/results_complete":
            _require(metrics, "required_result_keys", "actual_result_keys", "missing_result_keys", "unexpected_result_keys", "marker_states", "runtime_fingerprint")
            required = metrics["required_result_keys"]
            return (
                len(required) == 6
                and metrics["actual_result_keys"] == required
                and metrics["missing_result_keys"] == []
                and metrics["unexpected_result_keys"] == []
                and set(metrics["marker_states"]) == set(required)
                and all(value == "COMPLETE" for value in metrics["marker_states"].values())
                and _runtime_evidence_ok(metrics["runtime_fingerprint"])
            )
        if gid.startswith("G2.7/"):
            _require(metrics, "spread_pct", "agree_tolerance_pct", "official69_result", "charuco_result")
            first = _finite(metrics["official69_result"])
            second = _finite(metrics["charuco_result"])
            spread = abs(first - second) / min(first, second) * 100
            return first > 0 and second > 0 and abs(spread - _finite(metrics["spread_pct"])) <= 1e-9 and spread <= _finite(metrics["agree_tolerance_pct"])
        if gid == "G2.8":
            _require(metrics, "consensus", "spread_pct", "tolerance_pct")
            consensus = metrics["consensus"]
            if set(consensus) != {"1920x1080", "2688x1512", "3840x2160"}:
                return False
            values = [_finite(value) for value in consensus.values()]
            spread = (max(values) - min(values)) / min(values) * 100
            return min(values) > 0 and abs(spread - _finite(metrics["spread_pct"])) <= 1e-9 and spread <= _finite(metrics["tolerance_pct"])

    if stage == "S3_pairs":
        if gid == "G0.2":
            _require(metrics, "expected_lineage", "actual_lineage", "forbidden_lineage")
            return len(metrics["expected_lineage"]) == 7 and metrics["expected_lineage"] == metrics["actual_lineage"] and metrics["forbidden_lineage"] == []
        if gid == "G3.0":
        ...
        if gid == "G3.1":
            ...
            return metrics["n_camera_slots"] == metrics["n_non_null"] == 3
        if gid == "G3.2":
            _require(metrics, "n_images", "n_unique_images", "missing_seed_names", "dimension_mismatches")
            return metrics["n_images"] == metrics["n_unique_images"] == 1414 and metrics["missing_seed_names"] == [] and metrics["dimension_mismatches"] == []
        if gid == "G3.3":
            _require(metrics, "n_pairs", "n_images", "pair_density", "minimum")
            density = metrics["n_pairs"] / metrics["n_images"]
            return metrics["n_pairs"] >= 6000 and metrics["n_images"] == 1414 and abs(density - _finite(metrics["pair_density"])) <= 1e-12 and density >= _finite(metrics["minimum"])
        if gid == "G3.4":
            _require(metrics, "expected", "discovered")
            return len(metrics["expected"]) == 7 and metrics["expected"] == metrics["discovered"]
        if gid == "G3.5a":
            _require(metrics, "fwd", "rev", "expected")
            return len(metrics["fwd"]) == 4 and len(metrics["rev"]) == 3 and set(metrics["fwd"]).isdisjoint(metrics["rev"]) and set(metrics["fwd"]) | set(metrics["rev"]) == set(metrics["expected"])
        if gid == "G3.5b":
            _require(metrics, "n_pairs", "n_parsed", "n_sequence_pairs", "max_pairs", "per_video_pair")
            per_pair = metrics["per_video_pair"]
            return metrics["n_pairs"] == metrics["n_parsed"] == 6000 and metrics["n_sequence_pairs"] == len(per_pair) == 12 and metrics["n_pairs"] <= metrics["max_pairs"] and sum(row["n_pairs"] for row in per_pair.values()) == 6000
        if gid == "G3.5c":
        ...
        if gid == "G3.5d":
            ...
            return Path(metrics["images_path"]).is_absolute() and metrics["expected"] == metrics["injected"] == 6000 and metrics["missing_pairs"] == 0 and metrics["missing_endpoints"] == []
        if gid == "G3.6":
            _require(metrics, "config", "extra_pairs_path", "forced_pairs_sha256", "parsed_pairs", "injected_pairs", "refine_intrinsics", "skip_doppelgangers", "num_workers", "num_track_per_img", "sift_max_num_features", "max_num_tracks", "memory_safe_launcher")
            return Path(metrics["config"]).is_absolute() and Path(metrics["extra_pairs_path"]).is_absolute() and Path(metrics["memory_safe_launcher"]).is_absolute() and Path(metrics["memory_safe_launcher"]).is_file() and isinstance(metrics["forced_pairs_sha256"], str) and len(metrics["forced_pairs_sha256"]) == 64 and metrics["parsed_pairs"] == metrics["injected_pairs"] == 6000 and metrics["refine_intrinsics"] is False and metrics["skip_doppelgangers"] is False and type(metrics["num_workers"]) is int and metrics["num_workers"] == 0 and metrics["num_track_per_img"] == 512 and metrics["sift_max_num_features"] == 2048 and metrics["max_num_tracks"] == 400000
...
def validate_intrinsics_seed(run_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return _check(
            ...
            ok,
            "seed reopens as three PINHOLE cameras over the exact frame set"
            if ok
...
def validate_s2b_semantics(run_dir: Path) -> dict[str, Any]:
    """Reopen all six models and recompute completeness/intrinsics predicates."""
    try:
        import pycolmap
        ...
        return _check(
            ...
            not errors,
            "six models reopened and all completeness/spread predicates recomputed"
            if not errors
...
def validate_s3_semantics(run_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        log_text = (root / "s3_loader_probe.log").read_text(encoding="utf-8")
        with tempfile.TemporaryDirectory(prefix="target_site_s3_verify_") as temp:
        ...
        errors = []
        if len(pairs) != len(pair_set) or len(pair_set) != 6000:
            errors.append("forced text is not exactly 6000 unique pairs")
        if manifest.get("n_pairs") != 6000 or manifest.get("forced_pairs_sha256") != sha256(forced_path):
            errors.append("forced manifest does not bind the 6000-pair text")
        ...
        if "Forced pairs: 6000" not in log_text or "GT intrinsics loaded: 3/3" not in log_text:
            errors.append("probe log lacks 6000-pair or 3/3-camera evidence")
        if replay_drift:
            errors.append("persisted loader probe differs from independent replay")
        if not (
            replay.get("n_forced_expected") == replay.get("n_forced_injected") == 6000
            and replay.get("missing_forced_pairs") == 0
            and replay.get("n_gt_camera_slots") == replay.get("n_non_null_gt_cameras") == 3
        ):
            errors.append("independent loader replay is not 6000/6000 and 3/3")
        return _check(
            "release/material_semantic/S3_pairs",
            not errors,
            "S3 files reparse and independent loader replay is 6000/6000 with 3/3 GT cameras"
            if not errors
...
def validate_s0_semantics(run_dir: Path) -> dict[str, Any]:
    try:
        ...
        expected_test = {"T01_P1230123", "T02_P1260126"}
        ...
        if len(hashes) != 9 or len(set(hashes)) != 9:
            errors.append("nine corpus hashes are not present and unique")
        return _check(
            "release/material_semantic/S0_corpus",
            not errors,
            "corpus manifest independently resolves the exact 7+2 split"
            if not errors
...
def validate_s1_semantics(run_dir: Path) -> dict[str, Any]:
    try:
        ...
        if len(directions) != 7 or list(directions.values()).count("fwd") != 4 or list(directions.values()).count("rev") != 3:
        ...
        return _check(
            ...
            not errors,
            "motion manifest replays all records and exact 4/3 directions"
            if not errors
...
def validate_s2_manifest_semantics(run_dir: Path) -> dict[str, Any]:
    try:
        ...
        if manifest.get("n_frames") != len(frames) or len(frames) != 1414:
...
def validate_stage_chain(run_dir: Path) -> list[dict[str, Any]]:
    ...
    for stage, path in zip(STAGES, gate_paths, strict=True):
        ...
        emitted_ids = [item.get("id") for item in payload.get("checks", [])]
        exact = (
            ...
            and all(
                item.get("state") == "PASS" and item.get("ok") is True
...
def main() -> None:
    ...
    parser.add_argument("--run-name", default="target_site_v1")
