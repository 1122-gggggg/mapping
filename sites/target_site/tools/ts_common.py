#!/usr/bin/env python3
"""Shared definitions for the target_site gluemap -> EDM build.

The corpus split is declared HERE, once, and every stage imports it. Two of the
held-out test videos are byte-for-byte the same resolution and camera as four of
the build videos, so nothing geometric can tell them apart at any later stage --
path-based exclusion is the ONLY thing keeping them out of the map. Declaring the
split in one place, and asserting it at every stage, is the whole defence.
"""
...
DATA = Path("/media/cihcilab/新增磁碟區/sfm_system/data/target_site")
BUILD_ROOT = Path("/media/cihcilab/新增磁碟區/sfm_system/建圖/target_site")
...
# ---------------------------------------------------------------- the corpus
# 7 videos. All Parrot Anafi, same drone serial PI040416BA8L105488.
BUILD: list[Video] = [
    Video(
        "S01_ABrot",
        "base/map/A-B(中間有往下看和左看).MP4",
        1920,
        1080,
        30000 / 1001,
        "fwd",
    ...
    Video("S02_BA", "base/map/B-A.MP4", 1920, 1080, 30000 / 1001, "rev", "2026-05-16"),
    Video(
        "S03_BA2", "base/map/B-A(2).MP4", 3840, 2160, 30000 / 1001, "rev", "2026-03-15"
    ),
    Video(
        "S04_ab", "updates/build/a-b.MP4", 2688, 1512, 24000 / 1001, "fwd", "2026-06-24"
    ),
    Video(
        "S05_P1220122",
        "updates/build/P1220122.MP4",
        2688,
        1512,
        24000 / 1001,
        "unknown",
...
# NEVER enters the map. Same camera and resolution as S04-S07.
TEST: list[Video] = [
    Video(
        "T01_P1230123",
        "updates/test/P1230123.MP4",
        2688,
        1512,
        24000 / 1001,
        "unknown",
...
# Dropped: the only file with no Parrot metadata (a libx264 re-encode). Its
# aspect ratio is fine; it is out because HFOV is unverifiable -- a downscale and
# a centre-crop are indistinguishable from the file, and they imply different
# focal lengths.
DROPPED = {
    "base/map/A-B.MP4": "libx264 re-encode, no Parrot metadata, HFOV unverifiable"
}

# 512 stray 1920x1080 JPEGs with EXIF stripped, whose source video exists nowhere
# on the filesystem. Same resolution as build group A and no metadata to tell them
# apart, so any stage that globs base/** silently ingests them with plausible-
# looking intrinsics.
EXCLUDED_DIRS = [DATA / "base" / "validation" / "P0710071_frames"]
...
def assert_no_test_leakage(gate: Gate, images_dir: Path, gid: str = "G0.2") -> bool:
    ...
    The test videos share a camera and a resolution with four build videos, so by
