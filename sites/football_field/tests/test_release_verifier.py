    ...
    write_json(
        run_dir / "motion_manifest.json",
        {
            ...
            "directions": {
                ...
                for seq, direction in resolved.items()
                if seq not in {"S01_ABrot", "S02_BA"}
            },
        },
    )
...
def test_s3_g36_typed_predicate_rejects_unsafe_num_workers(num_workers) -> None:
    run_dir = release.RUNS / "target_site_v1"
    payload = json.loads(
        (run_dir / "gates" / "S3_pairs.json").read_text(encoding="utf-8")
    )
    g36 = next(check for check in payload["checks"] if check["id"] == "G3.6")
    if num_workers == "missing":
        g36["metrics"].pop("num_workers", None)
    else:
        g36["metrics"]["num_workers"] = num_workers

    check = release.validate_typed_stage_evidence("S3_pairs", payload)

    assert check["ok"] is False
    assert "G3.6" in check["evidence"]["errors"]
...
def test_s3_material_semantics_rejects_unsafe_num_workers(
    ...
    run_dir = release.RUNS / "target_site_v1"
    persisted_probe = release.read_json(run_dir / "s3_loader_probe.json")
    original_safe_load = yaml.safe_load

    def load_mutated_config(text):
        config = original_safe_load(text)
        if num_workers == "missing":
    ...
    monkeypatch.setattr(yaml, "safe_load", load_mutated_config)
    monkeypatch.setattr(
        s3,
        "real_loader_probe",
        lambda _run_dir, _output_path: deepcopy(persisted_probe),
    )

    check = release.validate_s3_semantics(run_dir)

    assert check["ok"] is False
    assert any("num_workers" in error for error in check["evidence"]["errors"])
...
def test_s3_g36_typed_predicate_rejects_unsafe_feature_budget(
    ...
    g36 = next(check for check in payload["checks"] if check["id"] == "G3.6")
    g36["metrics"][field] = unsafe_value
