...
WHAT THIS STAGE LEARNED (and it contradicted the plan)
  ...
  >5 deg/s manoeuvre in S01_ABrot -- the file whose own name promises a look-down
...
from ts_intrinsics import official69  # noqa: E402
...
#
# MEASURED on S01_ABrot (497 s, the file whose own name says it contains a
# look-down and a look-left):
...
def scan_video(v: Video) -> dict:
    ...
    # K at the working width. official69 is the probe model; the 3% spread between
    # intrinsics candidates cannot flip a rotation-vs-parallax call.
    cam = official69(WORK_W, int(round(WORK_W * v.height / v.width)))
...
def main() -> None:
    ...
    g.check(
        "G0.2",
        lineage_ok,
        "corpus manifest locks exactly the seven declared BUILD sources with hashes "
    ...
    if not args.skip_direction:
        # Two anchors, both known from their filenames: A-B is forward, B-A is
        # reverse. One anchor is not enough -- see resolve_direction().
        anchors = {"fwd": "S01_ABrot", "rev": "S02_BA"}
        log(f"resolving direction against anchors {anchors}")
        refs = {k: megaloc_descriptors(BUILD_BY_SEQ[s])[1] for k, s in anchors.items()}
        for v in BUILD:
            if v.seq in anchors.values():
                continue
            directions[v.seq] = resolve_direction(v, refs)
            d = directions[v.seq]
            log(f"  {v.seq}: "
                + "  ".join(f"vs {k}: rho={s['rho']:+.3f} sim={s['median_sim']:.3f}"
                            for k, s in d["scores"].items())
                + f"  -> {d['direction']}  (declared: {d['declared']})")
    ...
    # G1.5 -- POSITIVE CONTROL.
    #
    # S01's own filename (中間有往下看和左看) guarantees the flight contains a
    # look-down and a look-left. So the detector MUST see sustained high angular
    # rate there. If it cannot, it is broken and every number in this stage is junk.
    #
    # Note what this gate does NOT assert: that those manoeuvres are DEGENERATE.
    # They are not. Measured, every sustained rot > 5 deg/s run in S01 carries
    # 0.8-4.3 deg of parallax -- the gimbal moves while the drone keeps flying, so
    # the camera centre keeps moving and the baseline survives. An earlier version
    # of this gate assumed manoeuvre == pure rotation and would have thrown away
    # perfectly good structure. The control tests the DETECTOR, not the hypothesis.
    s01 = scans["S01_ABrot"]
    recs = s01["records"]
    runs, i = [], 0
    while i < len(recs):
        if np.isfinite(recs[i]["rot_deg_s"]) and recs[i]["rot_deg_s"] >= ROT_RATE_DEG_S:
            j = i
            while j < len(recs) and np.isfinite(recs[j]["rot_deg_s"]) \
                    and recs[j]["rot_deg_s"] >= ROT_RATE_DEG_S:
                j += 1
            if j - i >= 6:  # >= 1.5 s
                runs.append({
                    "t_start": recs[i]["t"], "t_end": recs[j - 1]["t"],
                    "rot_deg_s": float(np.median([r["rot_deg_s"] for r in recs[i:j]])),
                    "parallax_deg": float(np.median([r["parallax_deg"] for r in recs[i:j]])),
                })
            i = j
        else:
            i += 1
    g.check("G1.5", len(runs) >= 2,
            f"POSITIVE CONTROL: S01_ABrot (中間有往下看和左看) -> {len(runs)} sustained "
            f"slews >= {ROT_RATE_DEG_S} deg/s: "
            + "; ".join(f"{r['t_start']:.0f}-{r['t_end']:.0f}s "
                        f"({r['rot_deg_s']:.1f}deg/s, par {r['parallax_deg']:.2f}deg)"
                        for r in runs[:4])
            if runs else "-- DETECTOR IS BROKEN: found no rotation in a file that says it has one",
            n_slews=len(runs), slews=runs,
            degenerate=[r for r in runs if r["parallax_deg"] < ROT_MAX_PARALLAX_DEG])
    ...
    if not args.skip_direction:
        resolved = {v.seq: v.direction for v in BUILD}
        resolved.update({seq: d["direction"] for seq, d in directions.items()})
        unresolved = sorted(
            seq for seq, direction in resolved.items() if direction not in {"fwd", "rev"}
        )
        g.check("G1.4a", not unresolved,
                "all seven BUILD sequences resolved to fwd/rev" if not unresolved
                else f"unresolved direction: {unresolved}",
                resolved=resolved, unresolved=unresolved)
        conflict = [f"{d['seq']}: declared={d['declared']} measured={d['direction']}"
                    for d in directions.values()
                    if d["declared"] != "unknown" and d["declared"] != d["direction"]]
        g.check("G1.4b", not conflict,
                "measured directions agree with the filenames" if not conflict
