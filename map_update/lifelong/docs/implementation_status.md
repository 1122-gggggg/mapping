...
- Importable `BaseMap` adapter contract with a built-in COLMAP-compatible text/binary reader.
...
- Localizer 2D–2D to current-map 2D–3D lifting with stable-mask checks, unique landmark aggregation and multi-reference support.
...
- Actual localizer model inference. Connect it through `MatcherAdapter` or provide precomputed `.npz` matches.
...
- Real bridge-submap reconstruction. Existing mapping commands can be called through an external adapter and their output evaluated by this repository.
