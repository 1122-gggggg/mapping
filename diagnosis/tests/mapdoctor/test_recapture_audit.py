...
from mapdoctor.recapture.types import Availability, Backend, MetricValue, PoseDirectionCell
...
def test_nonfinite_diagnostic_metric_is_invalid() -> None:
    report = audit_cells((_cell("retrieval_recall_at_k"),), Backend.EDM)
...
def test_nonfinite_authorization_metric_is_invalid() -> None:
    report = audit_cells((_cell("fim_lambda_min"),), Backend.EDM)
